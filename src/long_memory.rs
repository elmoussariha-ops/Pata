...
fn causality_path(root: &Path) -> PathBuf {
    mem_root(root).join("patch_causality.tsv")
}
...
pub fn record_patch_delta(
    ...
    let ts = now_ts()?;
    fs::create_dir_all(mem_root(root)).map_err(|e| e.to_string())?;
    let mut cur = fs::read_to_string(causality_path(root)).unwrap_or_default();
    cur.push_str(&format!(
        "{ts}\t{patch_id}\tdelta\t{}\t{}\t{}\n",
        files.join(" | "),
        risk,
    ...
    fs::write(causality_path(root), cur).map_err(|e| e.to_string())
}

pub fn record_patch_validation(
    root: &Path,
    patch_id: &str,
    ok: bool,
    note: &str,
...
pub fn record_patch_failure_link(
    root: &Path,
    patch_id: &str,
    failure_id: &str,
) -> Result<(), String> {
    ...
    let mut cur = fs::read_to_string(causality_path(root)).unwrap_or_default();
    cur.push_str(&format!(
        "{ts}\t{patch_id}\tfailure\t-\t80\tlinked={}\n",
        sanitize(failure_id)
    ));
    fs::write(causality_path(root), cur).map_err(|e| e.to_string())
}

pub fn render_suspect_patches(root: &Path) -> String {
    let txt = fs::read_to_string(causality_path(root)).unwrap_or_default();
    if txt.trim().is_empty() {
        return "no causality data".to_string();
    }
    ...
    for l in txt.lines() {
        ...
        let patch = c[1].to_string();
        let kind = c[2];
        let weight = c[4].parse::<i32>().unwrap_or(0);
        let score = scores.entry(patch.clone()).or_insert(0);
        match kind {
            "delta" => *score += weight / 5,
            "validate" => *score += if weight > 0 { 45 } else { -15 },
            "failure" => *score += 35,
            _ => {}
        }
        reasons
    ...
    ranked
        ...
        .map(|(p, s)| {
            format!(
                ...
                p,
                s,
                reasons.get(&p).cloned().unwrap_or_default().join(" | ")
            )
        })
        .collect::<Vec<_>>()
        .join("\n")
...
fn load_causality(root: &Path) -> Vec<CausalityEvent> {
    fs::read_to_string(causality_path(root))
        .unwrap_or_default()
        .lines()
        .filter_map(|l| {
            let c = l.split('\t').collect::<Vec<_>>();
            ...
            Some(CausalityEvent {
                ts: c[0].parse().unwrap_or(0),
                patch: c[1].to_string(),
                kind: c[2].to_string(),
                files: c[3].to_string(),
                weight: c[4].parse().unwrap_or(0),
                note: c[5].to_string(),
            })
        })
        .collect()
...
pub fn render_causality(root: &Path) -> String {
    let mut events = load_causality(root);
    if events.is_empty() {
        return "no causality data".to_string();
    }
    ...
    events
        ...
        .map(|e| {
            format!(
                "ts={} patch={} kind={} files={} weight={} note={}",
                e.ts, e.patch, e.kind, e.files, e.weight, e.note
            )
        })
        .collect::<Vec<_>>()
        .join("\n")
...
pub fn render_explain_break(root: &Path) -> String {
    let ranked = render_suspect_patches(root);
    let top = ranked.lines().next().unwrap_or("none");
    if top == "none" || top == "no causality data" {
        return "no break explanation available".to_string();
    }
    let patch_id = top.split('\t').next().unwrap_or("unknown");
    let events = load_causality(root)
        .into_iter()
        .filter(|e| e.patch == patch_id)
        .collect::<Vec<_>>();
    let mut details = vec![format!("top_suspect={top}")];
    let mut files = BTreeSet::new();
    let mut failure_links = Vec::new();
    let mut validate = Vec::new();
    for e in events {
        if e.kind == "delta" {
            for f in e.files.split(" | ") {
                if !f.trim().is_empty() && f != "-" {
                    files.insert(f.to_string());
                }
            }
        }
        if e.kind == "failure" {
            failure_links.push(format!("{} ({})", e.note, e.ts));
        }
        if e.kind == "validate" {
            validate.push(format!("{} ({})", e.note, e.ts));
        }
    }
    details.push(format!(
        "files_touched={}",
    ...
    details.push(format!("validate_trace={}", validate.join(" | ")));
    details.push(format!("failure_links={}", failure_links.join(" | ")));
    details.join("\n")
}

pub fn render_recommend_rollback(root: &Path) -> String {
    let ranked = render_suspect_patches(root);
    let top = ranked.lines().next().unwrap_or("no causality data");
    if top == "no causality data" {
        return top.to_string();
    }
    let pid = top.split('\t').next().unwrap_or("unknown");
    format!(
        "recommend_rollback_candidate={pid}\nreason=highest_suspicion_from_patch_causality\nsafety=manual_review_required_no_auto_rollback"
    )
}
...
mod tests {
    ...
    #[test]
    fn causality_suspect_ranking_outputs_patch() {
        let root = std::env::temp_dir().join(format!(
            "pata-long-memory-causality-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_nanos()
        ...
        fs::create_dir_all(&root).unwrap();
        record_patch_delta(&root, "patch-a", &["src/main.rs".to_string()], 80).unwrap();
        record_patch_validation(&root, "patch-a", false, "failed").unwrap();
        let out = render_suspect_patches(&root);
        assert!(out.contains("patch-a"));
        assert!(out.contains("suspicion="));
    }

    #[test]
    fn causality_recommendation_mentions_candidate() {
        let root = std::env::temp_dir().join(format!(
            "pata-long-memory-causality-rec-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_nanos()
        ...
        fs::create_dir_all(&root).unwrap();
        record_patch_delta(&root, "patch-z", &["src/lib.rs".to_string()], 90).unwrap();
        record_patch_validation(&root, "patch-z", false, "break").unwrap();
        let r = render_recommend_rollback(&root);
        assert!(r.contains("patch-z"));
    }

    #[test]
    fn causality_explain_break_contains_trace() {
        let root = std::env::temp_dir().join(format!(
            "pata-long-memory-causality-explain-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
                .as_nanos()
        ...
        fs::create_dir_all(&root).unwrap();
        record_patch_delta(&root, "patch-x", &["src/main.rs".to_string()], 88).unwrap();
        record_patch_failure_link(&root, "patch-x", "fm-1").unwrap();
        record_patch_validation(&root, "patch-x", false, "validate failed").unwrap();
        let txt = render_explain_break(&root);
        assert!(txt.contains("top_suspect="));
        assert!(txt.contains("files_touched="));
    }
}

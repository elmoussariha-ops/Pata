...
fn run() -> Result<(), String> {
    ...
    match cmd.as_str() {
        ...
        "history" => command_history(&root, &rest),
        "benchmark" | "perf" => command_benchmark(&root, rest.first().cloned().unwrap_or_else(|| "main".to_string()), low_power),
        "export-report" => command_export_report(&root, rest.first().map(|s| s.as_str()).unwrap_or("digest"), rest.get(1).map(|s| s.as_str())),
        ...
        _ => Err("usage: pata [--low-power] [--verbose] [scan|retrieve <q> [--explain-retrieval]|plan <goal>|patch <goal>|review <id> [--explain-risk]|approve <id> [decision]|apply <id>|validate|status|end-session|daily-summary|resume-session|watch [cycles]|multiverse <goal> [2-4]|ast-fingerprint [function_name]|benchmark [query]|history <suspect|causality|explain-break|recommend-rollback>|export-report <digest|causality|suspect> [path.md]|memory <show|recent|open-loops [--priority|--recent]|lessons|daily|weekly|digest|failures|failure-recent|promote-failure <id>|explain-open-loop <id>|similar-functions <fn_name>|add-open-loop <category> <detail> [priority] [module] [impact]|resolve-open-loop <id>|add-lesson <category> <detail>>|doctor|smoke-test|low-power-status|ollama-check|ollama-status|model-status|demo|tui]".to_string()),
    }
}
...
fn command_patch(root: &Path, objective: String, low_power: bool) -> Result<(), String> {
    ...
    let _ = long_memory::record_patch_delta(root, &proposal.id, &review.files, review.risk.score);
...
fn command_history(root: &Path, rest: &[String]) -> Result<(), String> {
    let sub = rest.first().map(String::as_str).unwrap_or("suspect");
    match sub {
        "suspect" => {
            println!("== history suspect ==");
            println!("{}", long_memory::render_suspect_patches(root));
            Ok(())
        }
        "causality" => {
            println!("== history causality timeline ==");
            println!("{}", long_memory::render_causality(root));
            Ok(())
        }
        "explain-break" => {
            println!("== history explain-break ==");
            println!("{}", long_memory::render_explain_break(root));
            Ok(())
        }
        "recommend-rollback" => {
            println!("== history recommend-rollback ==");
            println!("{}", long_memory::render_recommend_rollback(root));
            Ok(())
        }
...
fn command_benchmark(root: &Path, query: String, low_power: bool) -> Result<(), String> {
    ...
    io_fast::clear_cache();
    let t0 = Instant::now();
    let idx_cold = scanner::scan_repo(root)?;
    let scan_cold = t0.elapsed().as_millis();

    let t1 = Instant::now();
    let idx_warm = scanner::scan_repo(root)?;
    let scan_warm = t1.elapsed().as_millis();

    io_fast::clear_cache();
    let t2 = Instant::now();
    let _r_cold = retriever::top_n(
        &idx_cold,
        &query,
        retrieval_limit(low_power, 6),
        &idx_cold.workspace_root,
    );
    let retrieve_cold = t2.elapsed().as_millis();

    let t3 = Instant::now();
    let _r_warm = retriever::top_n(
        &idx_warm,
        &query,
        retrieval_limit(low_power, 6),
        &idx_warm.workspace_root,
    );
    let retrieve_warm = t3.elapsed().as_millis();

    io_fast::clear_cache();
    let t4 = Instant::now();
    let fp_cold = ast_fingerprint::build_index(root)?;
    let fp_cold_ms = t4.elapsed().as_millis();

    let t5 = Instant::now();
    let fp_warm = ast_fingerprint::build_index(root)?;
    let fp_warm_ms = t5.elapsed().as_millis();
    ast_fingerprint::persist_index(root, &fp_warm)?;

    println!("== benchmark (query='{query}', low_power={low_power}) ==");
    println!("scan: cold={}ms warm={}ms", scan_cold, scan_warm);
    println!(
        "retrieve: cold={}ms warm={}ms",
        retrieve_cold, retrieve_warm
    );
    println!(
        "fingerprint: cold={}ms warm={}ms (functions={})",
        fp_cold_ms,
        fp_warm_ms,
        fp_cold.len()
    );
    println!(
        "watch_refresh_estimate={}ms",
        scan_warm + retrieve_warm + fp_warm_ms
    );
    Ok(())
}

fn command_export_report(root: &Path, kind: &str, path: Option<&str>) -> Result<(), String> {
    let out_path = path
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(format!("pata-{kind}-report.md")));
    let body = match kind {
        "digest" => long_memory::render_digest(root),
        "causality" => long_memory::render_causality(root),
        "suspect" => long_memory::render_suspect_patches(root),
        _ => return Err("unknown export-report kind".to_string()),
    };
    let md = format!("# Pata {kind} report\n\n```text\n{body}\n```\n");
    std::fs::write(&out_path, md).map_err(|e| e.to_string())?;
    println!("exported report: {}", out_path.display());
    Ok(())
}
...
fn command_apply(root: &Path, id: Option<&str>) -> Result<(), String> {
    ...
    if let Err(e) = patcher::apply(root, &proposal) {
        let fail_id = long_memory::record_failure(
            root,
            &proposal.objective,
            "apply",
            "apply_failed",
        ...
        .ok();
        if let Some(fid) = fail_id.as_deref() {
            let _ = long_memory::record_patch_failure_link(root, &patch_id, fid);
        }
        let _ = long_memory::record_patch_validation(root, &patch_id, false, &e);
    ...
    if !report.ok() {
        ...
        let _ = long_memory::record_patch_validation(
            root,
            &patch_id,
            false,
            "validate_after_apply_failed",
        );
        rollback::rollback(root, "HEAD~1")?;
        history::log(
            root,
    ...
    let _ = long_memory::record_patch_validation(root, &patch_id, true, "apply_validate_ok");
